...
- Version 0.3.4 uses SillyTavern's normal foreground generation path for the Story stage. The generated story message is therefore rendered and saved by SillyTavern, and follows the normal streaming setting when the selected API supports streaming. Outline and NPC drafts remain quiet structured requests so the workbench can parse them without inserting temporary messages into the chat.
- The Outline and NPC stages intentionally use SillyTavern's quiet generation API. In SillyTavern 1.17, quiet generation is explicitly excluded from the core streaming processor. Making those requests foreground generations would temporarily insert structured JSON/tag output into the chat and create unreliable cleanup behavior, so the extension does not claim a fake token stream for those stages.
